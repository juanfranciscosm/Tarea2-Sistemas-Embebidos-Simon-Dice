...
// Figuras para matriz 8x8
const uint8_t flecha_arriba[8] = {
    0B00011000,
    0B00111100,
    0B01111110,
    0B00011000,
...
const uint8_t flecha_izquierda[8] = {
    0B00010000,
    0B00110000,
    0B01111110,
    0B11111110,
...
const uint8_t flecha_derecha[8] = {
    0B00001000,
    0B00001100,
    0B01111110,
    0B01111111,
...
void max7219_init() {
    // Pines PB2, PB3 y PB5 como salidas
    ...
    max7219_send(0x0A, 0x08); // Intensidad media
    max7219_send(0x0B, 0x07); // Usar las 8 filas
    max7219_send(0x0C, 0x01); // Encender display
    max7219_send(0x0F, 0x00); // Modo test apagado
}
...
void mostrar_figura(const uint8_t figura[8]) {
    for (uint8_t i = 0; i < 8; i++) {
        max7219_send(i + 1, figura[i]);
    }
}

void enviar_comando_pic(uint8_t comando) {
    // Se envían los 4 bits bajos por PD0-PD3
...
void configurar_puertos() {
    // Comunicación al PIC: PD0-PD4 como salida
    DDRD |= 0B00011111;
    PORTD &= 0B11100000;

    // Botones PC0-PC3 como entradas
...
void mostrar_simbolo(uint8_t simbolo) {
    ...
    if (simbolo == 1) {
        mostrar_figura(flecha_arriba);
    } else if (simbolo == 2) {
        mostrar_figura(flecha_abajo);
    } else if (simbolo == 3) {
        mostrar_figura(flecha_izquierda);
    } else if (simbolo == 4) {
        mostrar_figura(flecha_derecha);
    }
}
...
void mostrar_nivel() {
    ...
    if (nivel == 1) {
        mostrar_figura(nivel_1);
        enviar_comando_pic(2);
    } else if (nivel == 2) {
        mostrar_figura(nivel_2);
        enviar_comando_pic(3);
    } else if (nivel == 3) {
        mostrar_figura(nivel_3);
        enviar_comando_pic(4);
    }
...
void mostrar_secuencia() {
    ...
    if (nivel == 1) {
        velocidad = demora_lenta;
    } else if (nivel == 2) {
        velocidad = demora_media;
    } else {
        velocidad = demora_rapida;
    }
